    ...
    def prune_and_fuse(self, gamma_thresh, verbose=False):
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(self.stem), 3):
                ...
                new_stem.append(
                    nn.Conv2d(
                        input_size,
                        out_size,
                        kernel_size=self.stem[i].kernel_size,
                        stride=self.stem[i].stride,
                        padding=self.stem[i].padding,
                    )
                )
                new_stem.append(nn.ReLU(inplace=True))
                new_stem[-2].weight.copy_(
                    torch.index_select(
                        torch.index_select(
                            W, 1, torch.nonzero(input_validity).squeeze()
                        ),
                        0,
                        torch.nonzero(out_validity).squeeze(),
                    )
                )
                ...
                if verbose:
                    print(
                        "Stem "
                        + str(len(new_stem) / 2 - 1)
                        + ": Pruned "
                        + str(len(out_validity) - out_size)
                        + " from "
                        + str(len(out_validity))
                    )
            ...
            for i in range(len(self.features)):
                ...
                new_features.append(
                    SelecSLSBlockFused(
                        inp,
                        skip,
                        int(sum(a_validity.int()).item()),
                        int(sum(b_validity.int()).item()),
                        int(sum(c_validity.int()).item()),
                        int(sum(d_validity.int()).item()),
                        int(sum(e_validity.int()).item()),
                        int(sum(out_validity.int()).item()),
                        self.features[i].isFirst,
                        self.features[i].stride,
                    )
                )
                ...
                new_features[i].conv1[0].weight.copy_(
                    torch.index_select(
                        torch.index_select(W, 1, torch.nonzero(i_validity).squeeze()),
                        0,
                        torch.nonzero(o_validity).squeeze(),
                    )
                )
                ...
                if verbose:
                    print(
                        ...
                        + str(i)
                        + ".conv1: Pruned "
                        + str(len(o_validity) - sum(o_validity.int()).item())
                        + " from "
                        + str(len(o_validity))
                    )
                # Conv2
                ...
                new_features[i].conv2[0].weight.copy_(
                    torch.index_select(
                        torch.index_select(W, 1, torch.nonzero(i_validity).squeeze()),
                        0,
                        torch.nonzero(o_validity).squeeze(),
                ...
                new_features[i].conv4[0].weight.copy_(
                    torch.index_select(
                        torch.index_select(W, 1, torch.nonzero(i_validity).squeeze()),
                        0,
                        torch.nonzero(o_validity).squeeze(),
                ...
                new_features[i].conv5[0].weight.copy_(
                    torch.index_select(
                        torch.index_select(W, 1, torch.nonzero(i_validity).squeeze()),
                        0,
                        torch.nonzero(o_validity).squeeze(),
                ...
                # Conv6
                i_validity = torch.cat(
                    [
                        a_validity.clone().detach(),
                        c_validity.clone().detach(),
                        e_validity.clone().detach(),
                    ],
                    0,
                )
                if self.features[i].isFirst:
                ...
                new_features[i].conv6[0].weight.copy_(
                    torch.index_select(
                        torch.index_select(W, 1, torch.nonzero(i_validity).squeeze()),
                        0,
                        torch.nonzero(o_validity).squeeze(),
            ...
            for i in range(len(self.head)):
                ...
                new_head.append(
                    nn.Conv2d(
                        input_size,
                        out_size,
                        kernel_size=self.head[i][0].kernel_size,
                        stride=self.head[i][0].stride,
                        padding=self.head[i][0].padding,
                    )
                )
                new_head.append(nn.ReLU(inplace=True))
                new_head[-2].weight.copy_(
                    torch.index_select(
                        torch.index_select(
                            W, 1, torch.nonzero(input_validity).squeeze()
                        ),
            ...
            new_classifier.append(
                nn.Linear(
                    int(sum(input_validity.int()).item()),
                    self.classifier[0].weight.shape[0],
                )
            )
            new_classifier[0].weight.copy_(
                torch.index_select(
                    self.classifier[0].weight,
                    1,
                    torch.nonzero(input_validity).squeeze(),
                )
            )
